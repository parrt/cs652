...
$\underbrace{\text{\tt f:}{\tt x~}{\tt [}\cdots [}_{\Delta \,=\,\#scopes} \cdots\, x {\tt :=} \,expr$ &
\begin{minipage}[t]{0.25\linewidth}
$expr$\\
{\tt store\_local} $\Delta, i$
\end{minipage} & \\

$\text{\tt f [}\cdots\underbrace{{\tt [|x| }\cdots [}_{\Delta}\cdots~ x {\tt :=} \,expr$ &
\begin{minipage}[t]{0.25\linewidth}
$expr$\\
{\tt store\_local} $\Delta, i$
\end{minipage} & \\
...
$expr$\\
{\tt send\_super} $n,i$
\end{minipage}  & $\text{\tt f}^{block_j}_{literal_i} = ``w_1{\tt :}w_2{\tt :}\cdots w_n{\tt :}$'' \vspace{3pt}\\

\hline 
99 & {\tt push\_int 99} & \\

{\tt 1.2} & {\tt push\_float floatToIntBits(1.2)} & \\

{\tt 'a string'} & {\tt push\_literal} $i$ & $\text{\tt f}^{block_j}_{literal_i} = ``a string$'' \\
